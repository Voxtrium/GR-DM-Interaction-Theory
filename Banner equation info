...
## Continuity system  
*(densities in \(\mathrm{GeV}^4\), comoving \(V_c\,[\mathrm{GeV}^{-3}]\), sources in \(\mathrm{GeV}^5\))*
\[
\begin{aligned}
\dot\rho_\Lambda &= \frac{\alpha_h}{V_c}\,\dot S_{\rm hor},\\[2pt]
\dot\rho_{\rm DM}+3H\rho_{\rm DM} &= p_{\rm DM}\,\frac{\varepsilon_h}{V_c}\,\dot S_{\rm hor},\\[2pt]
\dot\rho_{\rm GW}+4H\rho_{\rm GW} &= p_{\rm GW}\,\frac{\varepsilon_h}{V_c}\,\dot S_{\rm hor},\\[2pt]
...
\[
\sum_{i\in\{\Lambda,{\rm DM},{\rm GW},{\rm hor}\}}\!\!\left(\dot\rho_i+3H(1+w_i)\rho_i\right)=0\,.
\]
...
*(extensive in \(V_c\); \(\dot S_{\rm hor}\,[\mathrm{GeV}]\))*
\[
\dot S_{\rm hor}(t)=\int dM\,N(M,t)\,\dot S_{\rm BH}(M,t)
+\iint dM_1\,dM_2\,R_{\rm merg}(M_1,M_2,t)\,\Delta S_{\rm merg}.
\]
Here \(N\) is a count (dimensionless) inside \(V_c\); \(\dot S_{\rm BH}\) and \(R_{\rm merg}\) are rates \([\mathrm{GeV}]\); \(\Delta S_{\rm merg}\) is dimensionless.

---

## Micro-informed coefficients  *(both in \(\mathrm{GeV}\))*
\[
\alpha_h=C_\alpha\,\Big(\frac{\kappa}{K_s}\Big)\,\big(|\Omega|\,R_\ast\big), \qquad
\varepsilon_h=C_\varepsilon\,\Big(\frac{\kappa}{K_s}\Big)\,\big(|\Omega|\,R_\ast\big),
\]
...
- \([\dot S_{\rm hor}] = \mathrm{GeV}\), \([V_c] = \mathrm{GeV}^{-3}\), \([\alpha_h]=[\varepsilon_h]=\mathrm{GeV}\).
...
- \(1~\mathrm{Mpc}=1.563738286\times10^{38}~\mathrm{GeV}^{-1}\), hence \(1~\mathrm{Mpc}^3=3.823774\times10^{114}~\mathrm{GeV}^{-3}\).

---

## \(\Lambda\)CDM limit (sanity)
